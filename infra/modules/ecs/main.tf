##############################################################################
# ECS Module — Fargate Cluster, ALB, Service, Auto-Scaling
##############################################################################
...
variable "memory" {
  type    = number
  default = 2048 # 2 GB (ESM-2 + XGBoost need headroom)
}
...
# Allow pulling secrets from SSM
...
# ── IAM: Task Role (S3 access for the application) ──
